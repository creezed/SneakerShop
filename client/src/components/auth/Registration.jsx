...
const Registration = () => {
    ...
    return (
        <S.AuthForm onSubmit={handleSubmit(onSubmit)}>
            ...
            <Text as='h2' typography="xxxl">Создайте аккаунт</Text>
            <Text as='p' typography="m" TextColor="gray800">
                Зарегистрируйтесь на портале для дальнейшего оформления заказа и его отслеживания
            </Text>
            <Controller
                name={'name'}
                control={control}
            ...
            <S.AuthFooter>
                <Controller
                    ...
                    render={({ field }) =>
                        <Checkbox
                            {...field}
                            label={`Я соглашаюсь с Условиями обработки персональных данных и политикой конфиденциальности`}
                            gap="36px"
                            labelFz='xs'
                        />
                    }
            ...
            <Button type={ "submit" } disabled={Object.keys(errors).length} variant="fill" size='m' borderR='20px'>Создать</Button>
        </S.AuthForm>
    );
};
